...
from spins2.single import bilayer_ab_u_3_update
...
def run(file, init, X, Y, J, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if len(A) == 1:
        ...
        Aa, Ab = A[0], A[1]
    logging.info("{} {:<8} {} {:<8} {:<8} {:<8} {} {:<8} {:<8} {}".format("init:", init, "parameters(meV):", J0, J1, Ja, "anisotropy(meV):", Aa, Ab, "S=3/2"))
    if init == "fm":
        ...
        for i in range(lav):
            t, Nw, Ew = bilayer_ab_u_3_update.iteration3(latt, X_s, Y_s, J0, J1, Ja, Aa, Ab, arrays_values[i], nequilibrium, nworks)
            m_ave = functions.Average(latt[:,:,:,:,2])
